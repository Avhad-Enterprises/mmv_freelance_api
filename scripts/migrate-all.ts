    ...
    for (const schemaFile of migrationOrder) {
      ...
      try {
        ...
        if (
          ...
          error.code === "2BP01" &&
          error.message.includes("cannot drop table")
        ) {
          // Foreign key constraint issue - table has dependencies, skip drop and try to create
          console.log(
            `   🔗 ${schemaName} has dependencies, skipping drop but creating table`
          );
          ...
          console.log(
            `   ✨ ${schemaName} already exists, marking as migrated`
          );
          const version = getSchemaVersion(schemaPath);
          await recordMigration(schemaName, version);
          migratedCount++;
...
console.log(
  "   These indicate normal migration behavior and are handled automatically.\n"
);
