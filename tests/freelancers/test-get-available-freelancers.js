...
/**
 * Freelancers Get Available API Test
 * Tests the GET /freelancers/getavailablefreelancers endpoint
 */
...
/**
 * Test Get Available Freelancers
 */
async function testGetAvailableFreelancers() {
  printSection('Testing Get Available Freelancers');

  // Test 1: Get available freelancers without auth
  console.log('Test 1: Get available freelancers without authentication');
  console.log('📤 Request: GET /freelancers/getavailablefreelancers');
  console.log('🔑 Auth: None');

  const response1 = await makeRequest('GET', `${CONFIG.apiVersion}/freelancers/getavailablefreelancers`);
  ...
  printTestResult('Get available freelancers without auth', test1Passed, `Status: ${response1.statusCode}, Expected: 401`, response1);
  if (test1Passed) passedTests++; else failedTests++;

  // Test 2: Get available freelancers with valid auth
  console.log('\nTest 2: Get available freelancers with valid authentication');
  console.log('📤 Request: GET /freelancers/getavailablefreelancers');
  console.log('🔑 Auth: Bearer token (super admin)');

  const response2 = await makeRequest('GET', `${CONFIG.apiVersion}/freelancers/getavailablefreelancers`, null, authHeader('superAdmin'));
  ...
  printTestResult('Get available freelancers with auth', test2Passed, `Status: ${response2.statusCode}, Expected: 200`, response2);
  ...
  if (test2Passed && response2.body?.data) {
    ...
    const hasValidStructure = isArray && data.every(item =>
      ...
      item.hasOwnProperty('availability') &&
  ...
  // Test 4: Verify availability filter
  if (test2Passed && response2.body?.data) {
    console.log('\nTest 4: Verify availability filter');
    const data = response2.body.data;
    const allAvailable = data.every(item =>
      item.availability === 'full-time' ||
      item.availability === 'part-time' ||
      item.availability === 'freelance'
    );

    const test4Passed = allAvailable;
    printTestResult('Verify availability filter', test4Passed, `All items have valid availability: ${allAvailable}`, {
      sampleAvailabilities: data.slice(0, 3).map(item => item.availability)
    });
...
async function runTests() {
  console.log('🧪 FREELANCERS GET AVAILABLE API TESTS');
  console.log('======================================\n');
  ...
  // Run tests
  await testGetAvailableFreelancers();
