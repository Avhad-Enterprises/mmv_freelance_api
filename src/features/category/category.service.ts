...
class CategoryService {
  ...
  public async addtocategory(data: CategoryDto): Promise<any> {
    ...
    try {
      ...
      const existingCategory = await DB(T.CATEGORY)
        .whereRaw("LOWER(TRIM(category_name)) = ?", [
      ...
      if (existingCategory) {
        if (!existingCategory.is_deleted) {
          throw new HttpException(
            400,
            "This category already exists in the database"
          );
        } else {
  ...
  public async getallcategorysbytable(): Promise<any> {
    try {
      ...
      const errorMessage =
        error.sqlMessage || error.message || "Error fetching categories";
  ...
  public async updatecategoryid(data: Partial<CategoryDto>): Promise<any> {
    ...
    if (!updated.length)
      throw new HttpException(404, "Category not found or not updated");
  ...
  public async SoftDeletecategory(data: Partial<CategoryDto>): Promise<any> {
    ...
    if (!deleted.length)
      throw new HttpException(404, "Category not found or not delete");
