...
class BlogService {
  /**
   * Get all blogs with optional active filter
   */
  public async getAllBlogs(
    ...
    try {
      ...
      const result = await DB(T.BLOG)
        .where({ is_active: isActive, is_deleted: false })
        .select("*")
  ...
  public async getBlogById(blog_id: number): Promise<any> {
    ...
    const blog = await DB(T.BLOG).where({ blog_id, is_deleted: false }).first();
  ...
  public async createBlog(data: BlogDto): Promise<any> {
    ...
    if (!data.title || !data.slug || !data.author_name) {
      throw new HttpException(
        400,
        "Title, slug, and author_name are required fields"
      );
    }
  ...
  public async updateBlog(data: Partial<BlogDto>): Promise<any> {
    ...
    if (!data.blog_id)
      throw new HttpException(400, "Blog ID is required for update");
    ...
    if (!updated.length)
      throw new HttpException(404, "Blog not found or not updated");
  ...
  public async deleteBlog(data: Partial<BlogDto>): Promise<any> {
    ...
    if (!data.blog_id)
      throw new HttpException(400, "Blog ID is required for deletion");
    ...
    if (!deleted.length)
      throw new HttpException(404, "Blog not found or not deleted");
