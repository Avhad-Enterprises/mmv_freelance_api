...
class BlogController {
  ...
  public getAllBlogs = async (
    ...
    try {
      ...
      // Explicitly convert to string and lower case to handle 'false' string correctly
      const is_active =
        String(isActiveQuery).toLowerCase() === "false" ? false : true;

      const blogs = await this.blogService.getAllBlogs({ is_active });
      res.status(200).json({
        success: true,
        data: blogs,
